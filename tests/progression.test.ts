...
describe("applyResult", () => {
  it("increments all sets when every set hits the target", () => {
    ...
    applyResult(logged([5, 5, 5, 5, 5]), squat, data.settings);
    expect(squat.sets.map((s) => s.weight)).toEqual([102.5, 102.5, 102.5, 102.5, 102.5]);
  });

  it("keeps the weight when a set is missed", () => {
    const { data, squat } = sampleData();
    applyResult(logged([5, 5, 5, 4, null]), squat, data.settings);
    expect(squat.sets.every((s) => s.weight === 100)).toBe(true);
  });

  it("saves the weight used in the workout", () => {
    const { data, squat } = sampleData();
    applyResult(logged([3, 3, 3, 3, 3], 110), squat, data.settings);
  ...
  it("checks and increments only the last set when asked", () => {
    ...
    applyResult(logged([1, 1, 1, 1, 5]), squat, data.settings);
    expect(squat.sets.map((s) => s.weight)).toEqual([100, 100, 100, 100, 102.5]);
  });

  it("uses the exercise override over global settings", () => {
    const { data, squat } = sampleData();
    squat.increment = { enabled: true, step: 5, targetReps: 3 };
    applyResult(logged([3, 3, 3, 3, 3]), squat, data.settings);
    expect(squat.sets[0]?.weight).toBe(105);
    squat.increment.enabled = false;
    applyResult(logged([5, 5, 5, 5, 5], 105), squat, data.settings);
    expect(squat.sets[0]?.weight).toBe(105);
  });
